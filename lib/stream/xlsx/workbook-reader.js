...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    // Read entire stream into buffer and use fflate unzipSync for efficient decompression
    const chunks = [];
    ...
    for (const [entryPath, fileData] of sortedEntries) {
      // Normalize path by removing leading slash (fflate returns paths with leading slash)
      const normalizedPath = entryPath.startsWith('/') ? entryPath.slice(1) : entryPath;
