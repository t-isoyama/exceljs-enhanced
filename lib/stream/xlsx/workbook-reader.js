...
// Performance: Use native os.tmpdir() and crypto instead of tmp package (Node 22+)
const os = require('os');
...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    for await (const entry of zip) {
      ...
      switch (entry.path) {
        ...
        case 'xl/sharedStrings.xml':
          await this._parseSharedStrings(entry);
          break;
        case 'xl/styles.xml':
        ...
        default:
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              // Manually collect entry data without using pipe() to avoid breaking ZIP iterator
              const chunks = [];
              ...
              // Performance: Write buffer to temp file using native fs (no tmp package)
              const tempFilePath = path.join(os.tmpdir(), `exceljs-${randomUUID()}.xml`);
              await fs.promises.writeFile(tempFilePath, buffer);
              waitingWorkSheets.push({
                sheetNo,
                path: tempFilePath,
                // Performance: Cleanup function compatible with previous tmp.file() API
    ...
    for (const {sheetNo, path: tempFilePath, tempFileCleanupCallback} of waitingWorkSheets) {
