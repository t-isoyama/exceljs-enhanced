...
// Performance Phase 4-1: Use unzipper for Node.js (better streaming), JSZip for browser
let unzipper;
try {
  // Only load unzipper in Node.js environment (not in browser builds)
  if (!process.browser) {
    unzipper = require('unzipper');
  }
} catch (e) {
  // Fallback to JSZip if unzipper is not available
  unzipper = null;
}
      ...
      for (const entry of directory.files) {
        ...
        if (entry.type !== 'Directory') {
          ...
          if (
            ...
            // running in node.js - use string directly
