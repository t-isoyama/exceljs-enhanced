...
class XLSX {
  ...
  reconcile(model, options) {
    ...
    // Performance Phase 4-5: for-of instead of forEach (5-10% improvement)
  ...
  async load(data, options) {
    ...
    // Performance Phase 2-2: Use fflate for unified ZIP reading (Node.js & browser)
    ...
    for (const [filename, fileData] of Object.entries(files)) {
      ...
      // Create PassThrough stream matching original unzipper pattern
      let stream;
      if (
        entryName.match(/xl\/media\//) ||
        entryName.match(/xl\/theme\/([a-zA-Z0-9]+)[.]xml/)
      ) {
        // Binary data (images, themes) - use regular PassThrough
        stream = new PassThrough();
        stream.end(Buffer.from(fileData));
      } else {
        // XML data (text) - use object mode PassThrough
        // Try simpler approach: write all content at once without chunking
        stream = new PassThrough({
      ...
      // Must await the processing to complete before moving to next entry
  ...
  // Performance Phase 4-1: Extract entry processing logic to avoid duplication
  async _processEntry(stream, model, entryName, options) {
    switch (entryName) {
      case '_rels/.rels':
