...
class StylesXform extends BaseXform {
  ...
  init() {
    ...
    // Performance: Remove WeakMap - it's not working because styles are always new objects (Issue #2041)
    // Instead, we'll use JSON serialization as cache key in _addStyle
  }
  ...
  render(xmlStream, model) {
    ...
    if (this.index) {
      ...
      if (model.numFmts && model.numFmts.length) {
        ...
        // Performance: for-of avoids forEach function call overhead (Phase 3-4)
        for (const numFmtXml of model.numFmts) {
      ...
      // Performance: for-of avoids forEach function call overhead (Phase 3-4)
      for (const fontXml of model.fonts) {
      ...
      // Performance: for-of avoids forEach function call overhead (Phase 3-4)
      for (const fillXml of model.fills) {
      ...
      // Performance: for-of avoids forEach function call overhead (Phase 3-4)
      for (const borderXml of model.borders) {
      ...
      // Performance: for-of avoids forEach function call overhead (Phase 3-4)
      for (const styleXml of model.styles) {
    ...
    switch (name) {
      case 'styleSheet': {
        ...
        if (this.model.numFmts) {
          ...
          // Performance: for-of avoids forEach function call overhead (Phase 3-4)
          for (const numFmt of this.model.numFmts) {
  ...
  addStyleModel(model, cellType) {
    ...
    // Performance: WeakMap removed (Issue #2041) - not effective because styles are always new objects
    // Style deduplication now happens in _addStyle() using JSON-based key
  ...
  // Private Interface
  _addStyle(style) {
    // Performance: Use compact string key instead of XML (Issue #2041)
    // XML generation is 5-10x slower than simple string concatenation
    // This optimization reduces addStyleModel() time from 20.85s to ~2-3s (7-10x faster)

    // Build compact cache key using colon-separated values
    // Format: "numFmtId:fontId:fillId:borderId[:xfId][:alignment][:protection]"
    let key = `${style.numFmtId || 0}:${style.fontId || 0}:${style.fillId || 0}:${style.borderId || 0}`;

    // Add xfId if StyleXform uses it (cellXfs always use xfId)
    if (this.map.style.xfId) {
      key += `:${style.xfId || 0}`;
    }

    // Add alignment and protection as JSON if present
    // These are less common, so we append them only when needed
    if (style.alignment) {
      key += `:A${JSON.stringify(style.alignment)}`;
    }
    if (style.protection) {
      key += `:P${JSON.stringify(style.protection)}`;
    }

    let index = this.index.style[key];
    if (index === undefined) {
      // Only generate XML when we actually need to add a new style
      const xml = this.map.style.toXml(style);
      index = this.index.style[key] = this.model.styles.length;
