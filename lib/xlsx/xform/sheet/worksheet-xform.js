...
const mergeRule = (rule, extRule) => {
  ...
  for (const key of Object.keys(extRule)) {
...
const mergeConditionalFormattings = (model, extModel) => {
  ...
  for (const extCf of extModel) {
    for (const extRule of extCf.rules) {
  ...
  reconcile(model, options) {
    ...
    const rels = (model.relationships || []).reduce((h, rel) => {
      ...
      if (rel.Type === RelType.VmlDrawing && model.comments && model.comments.length) {
        ...
        // Performance: for loop with index avoids forEach function call overhead
        for (let index = 0; index < model.comments.length; index++) {
