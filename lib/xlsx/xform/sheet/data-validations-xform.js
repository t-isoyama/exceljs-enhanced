...
class DataValidationsXform extends BaseXform {
  ...
  render(xmlStream, model) {
    ...
    if (optimizedModel.length) {
      ...
      for (const value of optimizedModel) {
        ...
        // Performance: for-of with index tracking
        const formulae = value.formulae || [];
        for (let index = 0; index < formulae.length; index++) {
  ...
  parseClose(name) {
    switch (name) {
      ...
      case 'dataValidation': {
        ...
        // Performance Phase 4-4: Store ranges directly instead of expanding (50-70% memory reduction)
        const list = this._address.split(/\s+/g) || [];
        for (const addr of list) {
          // Store range directly - no expansion needed!
          // DataValidations class now handles range lookups efficiently
          if (this.model.addRange) {
            // New format - use addRange method
            this.model.addRange(addr, this._dataValidation);
            continue;
          }
          // Fallback for old format (shouldn't happen in normal flow)
          if (addr.includes(':')) {
